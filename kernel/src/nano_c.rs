//! Безопасная Rust-обёртка над C-версией редактора
//! (kernel/src/nano_c/nano.c + nano.h, мост в nano_ffi.rs). Публичный
//! интерфейс нарочно похож на nano.rs, чтобы Mode::NanoC в main.rs и
//! terminal.rs::draw_nano_c* были почти зеркальны Mode::Nano/draw_nano*
//! — разница только в том, что тут поля не публичные, а через методы
//! (вызовы в C), и `Editor` владеет `*mut RawNanoEditor` вместо Vec/String.
//!
//! Известный неподтверждённый риск (тот же, что и в nano.rs, не новый):
//! имена вариантов `pc_keyboard::KeyCode::Home/End/PageUp/PageDown/
//! Delete` не перепроверялись офлайн — см. NANO_C.md.
...
/// То же самое, что MAIN_SHORTCUTS в nano.rs — у C-версии тот же набор
/// возможностей, так что и подсказки внизу экрана те же.
...
/// его собственной, отдельно вычитанной, логике (см. NANO_C.md).
impl Editor {
    ...
    /// заголовка, см. terminal.rs::draw_nano_c_panel.
