...
ASCII-шрифтом, прерывания+клавиатура, куча ядра). Начата **Phase 6/7/8**:
простая RAM-файловая система (`fs.rs`) и шелл (`shell.rs`) с командами
`ls cat rm mkdir touch cd pwd echo clear help` — работают по-настоящему,
не заглушки. Панель терминала теперь показывает прокручиваемую историю
вывода, а не два захардкоженных демо-текста. `file-sys` пока алиас на
`ls` — полноценный интерактивный TUI-файловый менеджер со стрелками
ещё предстоит.
...
Phase 4 доделать (иконки Nerd Font и кириллица в атласе) → интерактивный
`file-sys` (отдельный режим ввода: стрелки листают список, Enter входит
в папку, Esc/`q` выходит в шелл) → Phase 5 (CSS-подобный конфиг тем) →
история команд по `↑`/`↓` → персистентное хранилище вместо RAM-FS.
